...
前端文案统一通过 `web/src/i18n` 管理，当前支持 `zh-CN` 和 `en-US`。新增页面、弹窗、按钮、表单提示、空状态、确认信息、表头、枚举显示和路由标题都不能在页面组件中直接硬编码展示文案。
...
- API 结构化校验错误由前端请求层按字段 key 翻译；后端业务错误当前仍可能返回原始中文，后续需要完整国际化时优先改为稳定错误码加前端翻译。
