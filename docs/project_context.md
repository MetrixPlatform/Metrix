...
- 数据库脚本详情/原脚本更新/表结构设计器增强后，`pytest server/tests/test_database.py -q`、`npm run typecheck` 通过，ReadLints 无新增诊断。
