...
- 数据库标识符处理：库/schema 名仍使用严格标识符校验（字母、数字、下划线、`$`），保持现有库名约束；表名、字段名、索引名允许中文、横杠等需要引用的合法名称，但所有 SQL 拼接必须继续通过 `ExternalDatabase.quote_identifier()` 统一加反引号/双引号并转义。不要在表数据、行编辑、建表、改表、索引、排序字段中重新使用窄正则拦截表名/字段名，否则会导致 `fdd外部邻区-eutranrelation` 这类真实表名报 `Invalid table`。字段类型允许 `COLLATE "..."` 这类元数据后缀，但仍禁止分号等危险字符。
...
- 表名/字段名宽松标识符支持后，新增中文 + 横杠表名和中文字段名回归，`pytest server/tests/test_database.py -q` 通过。
