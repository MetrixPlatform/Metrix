...
- 系统设置页从左侧大表单、右侧大备份空白卡片改为主配置区加右侧工具区布局，页面自身撑满后台工作区，内部设置内容独立滚动。
- 设置项按“基础设置、注册与账号、API 与 Token、日志策略”拆分为紧凑分组面板，数据备份保留在右侧低频操作面板，减少空白并方便后续继续追加设置分组。
- 语言包补充设置分组说明文案；开发指南记录系统设置页后续新增设置应优先按紧凑分组扩展，避免回到大面积留白布局。
