...
- **提交范围偏好**：用户已明确要求，后续提交时默认把当前修改区里的实际改动一并提交，不再按“是否本轮修改”拆分；仍需遵守必要的状态/差异检查、敏感信息过滤和非空提交规则。
...
- dev.py 四进程为本机联调入口；Windows 下子进程使用 `CREATE_NEW_PROCESS_GROUP | CREATE_NO_WINDOW`，避免 `uvicorn --reload`/Vite 子进程重启时把控制事件传给父 `dev.py` 导致整套联调被误关停。
...
## 2026-06-24：CapacityReport 子模块化 + dev.py 多服务 + 数据库工作台/导数任务/容器引擎界面优化

CapacityReport 由 `temp/` 内的克隆转为 Metrix 的正式 git 子模块，并附带 4 项 Metrix 界面/能力优化（任务 6/7 是 CapacityReport 自身改动，记录在其仓库 `docs/project_context.md`，随 `metrix-integration` 分支提交）。

- 子模块（任务1）：CapacityReport 在自身仓库新建 `metrix-integration` 分支提交全部双模式 + 精简改动并推送 `github.com/nixevol/CapacityReport`；工作副本移动到 `e:/code/Metrix/CapacityReport`，根级 `.gitmodules`（url + `branch=metrix-integration`）+ gitlink（mode 160000）登记为子模块。新克隆需 `git submodule update --init` 拉取。`node_modules`/`.venv`/`frontend/dist` 等仍由 CapacityReport 自身 `.gitignore` 排除。
- dev.py（任务2）：在 Metrix backend(8000)/web(5173) 之外新增 `[capa-api]` CapacityReport 后端（`CapacityReport/.venv` 跑 `uvicorn app.main:app --reload --port 9081`，venv 缺失时按 `requirements.txt` 自动创建）与 `[capa-web]` 前端（`CapacityReport/frontend` vite `--port 5174`，缺 `node_modules` 自动 `npm install`）。`_wait_for_backend` 泛化为 `_wait_for_http(name,url)`；主循环改为仅 Metrix backend/web 退出才整体关停，CapacityReport（可选子模块）缺失或崩溃只记日志、不影响 Metrix。CapacityReport 的 `/health` 与 vite 代理(→9081)沿用其原配置。
- 数据库工作台只读状态（任务3）：后端 `engines.py` 加 `ExternalDatabase.server_version()`（mysql/mariadb 走 `SELECT VERSION()`、sqlite 走 `sqlite_version()`），`services.py` 加 `server_info()`，`schemas.py` 加 `DatabaseServerInfo{db_type,version,load_data_infile}`，`api.py` 加 `GET /api/databases/{conn_id}/server-info`（`DATABASE_READ`，database-data 隐藏分组）。前端 `api.ts` 加 `getDatabaseServerInfo`，`SqlWorkbenchView.vue` 左侧树下方加 `.database-server-info` 状态块（进入连接异步拉取，展示「版本」「快速导入(LOAD DATA) 可用/未启用」），`main.css` 加样式，database i18n 加 `serverInfo.*`。
- 导数任务筛选 + 未读角标（任务4）：列表 `DataJobsView.vue` 给「类型/状态/创建人」列加 naive 列筛选（`filterOptions/filterOptionValue/filterMultiple:false/filter:true` + `@update:filters`→`singleFilterValue`），连后端 `list_jobs` 已有的 `kind/status/created_by`(me/others)。角标由「待下载导出数」改为「未读已结束任务数」：新增 `data_job_views(user_id PK, seen_at)` 表（`models.py`，`Base.metadata.create_all` 自动建表），`repositories.py` 加 `get_seen_at/mark_seen/count_unseen`，`jobs.py` 加 `unseen_count()`（按 `DATABASE_MANAGE_OTHERS` 决定可见范围，统计 `finished_at>seen_at` 的 success/failed）与 `mark_seen()`，`api.py` 加 `GET /unseen-count`、`POST /mark-seen`。前端 `api.ts` 加 `getDataJobUnseenCount/markDataJobsSeen`（删除已无前端调用的 `getDataJobDownloadCount`，后端 `download-count` 端点与其测试保留），`DatabaseManageView.vue`/`SqlWorkbenchView.vue` 角标改用未读计数，`DataJobsView.vue` 挂载即 `markDataJobsSeen` 清零；回到列表/工作台时重新拉取→归零。i18n 加 `database.creatorOthers`，回归用例 mock 改为 `unseen-count`/`mark-seen`/`server-info`。原 `downloaded_at` 清理逻辑保持不动。
- 容器引擎区可收起（任务5）：`ContainerManageView.vue` 顶部 `.container-engine-card` 改为 `n-collapse/n-collapse-item`（默认收起），收起态头部显示标题 + 连接状态 `n-tag`（可用/不可用）+ 刷新按钮（`@click.stop` 不触发展开），展开显示 docker host/版本/OS/告警；展开态用 `localStorage`(`metrix.containers.engineExpanded`) 记忆；`main.css` 调整为 `.container-engine-collapse/.container-engine-header/.container-engine-body`。
- 验证：Metrix `web` `npm run build`（vue-tsc）通过、`server/app` `compileall` 通过、`pytest server/tests/test_database.py` 5 passed；CapacityReport `frontend` `npm run build` 通过、`app` `compileall` 通过，`metrix-integration` 已推送，根 `git submodule status` 指向该分支 HEAD。dev.py 四进程为本机联调入口（需各自 venv/node_modules，未在此环境实跑）。

## 2026-06-24：修复 dev.py 子服务 reload 误关停父进程

- 现象：`CapacityReport/app/config.py` 等文件变更触发 `[capa-api] uvicorn --reload` 后，父级 `dev.py` 收到类似 Ctrl+C 的控制事件，随后执行 `_stop_all()`，把 Metrix backend/web 与 CapacityReport api/web 全部关停。
- 修复：`dev.py::_spawn` 在 Windows 下创建子进程时使用 `CREATE_NEW_PROCESS_GROUP | CREATE_NO_WINDOW`。子服务 stdout/stderr 仍通过管道汇总到同一窗口，但 reload/HMR 子进程不再共享父控制台控制事件，避免子进程重启误触发父进程 `KeyboardInterrupt`。
- 验证：`python -m py_compile dev.py` 通过。

## 2026-06-24：用户级 MySQL MCP 开启写操作配置

- 用户级 `~/.cursor/mcp.json` 的 `mcp_server_mysql` 使用 `npx -y @benborla29/mcp-server-mysql`，连接本机 MySQL，并启用多库写模式与 `mysql_query` 的 DDL/INSERT/UPDATE/DELETE 权限。
- 配置中补充 `MYSQL_DB=""`（多库模式）以及 `capacityreport/relation/celldata/sftpgo` 的 schema 级 DDL/增删改权限；敏感连接信息只保存在用户级 Cursor 配置，不写入项目仓库。
- 注意：Cursor 通常需要重新加载 MCP server/重启窗口后，新的 tool 描述（如 `mysql_query`）才会在当前会话可用。

## 2026-06-24：更新 CapacityReport 子模块指针（CellData 配置入口）

- CapacityReport 子模块新增 CellData 远程 FTP/SFTP 与 CellData MySQL 配置入口，主仓库 gitlink 更新到 `metrix-integration` 的对应提交。
- 后续若推送主仓库该指针，仍需同步推送 CapacityReport `metrix-integration`，否则远端无法解析新的子模块 commit。

## 2026-06-24：精简 Web 用户界面说明文案

- Metrix Web 精简系统设置、脚本环境、容器配置、Token 与 OpenAPI 参数说明中的开发/实现细节文案，保留用户可理解的短提示。
- CapacityReport 子模块同步精简系统设置与历史页文案，去掉“后续流程”“缓存”“处理链路”等内部实现表达；主仓库子模块指针更新到对应提交。
- 验证：Metrix `web` `npm run build` 通过；CapacityReport `frontend` `npm run build` 通过；构建产物已清理。

## 2026-06-25：更新 CapacityReport 子模块指针（数据管理切换 CellData 库）

- CapacityReport 数据管理页支持在主数据库和 CellData 数据库之间切换，表列表、表结构、分页数据、清空/删除/导出等操作按当前选择的数据库执行。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（数据库选择弹窗）

- CapacityReport 数据管理页的数据库切换从下拉框改为图标按钮打开弹窗列表，列表固定高度并支持滚动，便于后续扩展更多数据库配置。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（简化数据管理标题）

- CapacityReport 数据管理页左侧标题去掉“表”字和具体库名，只显示分类标题（主数据库 / CellData），数据库选择仍通过图标按钮打开弹窗列表。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（数据库弹窗显示真实库名）

- CapacityReport 数据管理页左侧仍只显示分类标题（主数据库 / CellData），数据库选择弹窗内恢复显示真实库名（如 `主数据库：CapacityReport`）。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：用户级 SFTP MCP 开发调试配置

- 用户级 `~/.cursor/mcp.json` 新增 `sftp_dev`，使用 `npx -y mcp-server-ssh`，用于开发期通过 MCP 连接本机 SFTP（如 `127.0.0.1:2022`）查看/读取 CellData 来源文件。
- 该 MCP 包提供 `ssh_connect` 与 `sftp_*` 工具；凭据可在连接工具调用时传入，未写入项目仓库。Cursor 需要重新加载 MCP server/Reload Window 后才会生成对应工具描述。

## 2026-06-25：更新 CapacityReport 子模块指针（CellData cellinfo 映射文档）

- CapacityReport 记录了 CellData 原始 SFTP 路径、`Result_300_*.zip` 文件时间取名规则、ZIP 内核心 CSV 文件前缀、CSV 编码注意事项、`cellinfo` 字段映射以及 `CGI=PLMN-eNodeBID-CellID` 生成规则。
- 本轮仅记录处理依据，未实现 CellData 入库流水线；主仓库 gitlink 更新到对应子模块提交。

## 2026-06-25：更新 CapacityReport 子模块指针（CellData 预处理）

- CapacityReport 实现 CellData 预处理：路径模板与 `{maxyear}` 等占位符、正则配置、映射 JSON、最新 `Result_300` ZIP 选择、CSV 解析、清空并写入 `cellinfo`。
- 数据处理页新增 CellData 单独刷新入口；本地上传、远程手动和自动调度处理前也会执行 CellData 预处理，并复用全局任务锁避免并发写库。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（CellData 文件夹上传）

- CapacityReport 数据处理页的 CellData 卡片支持点击/拖拽文件夹上传处理，右上角提供说明弹窗；不再支持单 ZIP 直接拖入，避免无法识别 700M/2.6G 等频段目录。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（数据处理卡片说明）

- CapacityReport 记录了数据处理页容量数据与 CellData 卡片的说明按钮、文件夹上传约束和并排布局规则。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（远程数据源页收纳规则设置）

- CapacityReport 远程数据源页将 CellData 扫描路径、正则和映射 JSON 收纳到「规则设置」弹窗，JSON 使用 Monaco 编辑器并支持格式化/校验/恢复默认。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（CellData 扫描路径布局）

- CapacityReport 去掉 CellData 数据源的远程目录输入，将扫描路径维护放回数据源卡片；规则弹窗保留高级正则和映射 JSON。占位符扩展支持 `{maxmonth}`、`{maxday}`。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（扫描路径排版）

- CapacityReport 优化 CellData 数据源卡片中的扫描路径区域排版，路径列表和添加输入全宽对齐，减少配置区拥挤。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（CellData 扫描路径最终规则）

- CapacityReport 记录 CellData 数据源不再展示远程目录，扫描路径控制实际文件位置，并支持 `{maxyear}`、`{maxmonth}`、`{maxday}` 等占位符及对应目录正则。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（CellData 图形化映射）

- CapacityReport CellData 映射规则支持图形化 / JSON 双模式编辑，图形化模式可维护来源、CSV 前缀、字段映射和固定值，JSON 模式继续使用 Monaco。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（CellData 映射弹窗滚动）

- CapacityReport CellData 规则设置弹窗固定高级匹配、目标表与主键信息，仅来源列表滚动，添加来源和保存按钮始终可见。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（图形化来源折叠）

- CapacityReport CellData 图形化映射中每个来源支持展开/收起，添加来源按钮放到映射规则标题区右上角。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（LTE_SDR 小区名称映射）

- CapacityReport 修正 CellData 默认映射中 `LTE_SDR_CellInfo` 的小区名称来源字段为 `cellName`，解决 4G 小区名称为空的问题。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：新增本轮详细交接文档

- 新增 `docs/handoff_2026-06-25.md`，汇总本轮对话中的 CapacityReport、CellData、MCP、数据处理页、远程数据源页、映射规则、提交状态和后续开发入口。
- 新窗口可先阅读该文件，再读 `docs/project_context.md` 顶部总览与 `CapacityReport/docs/project_context.md` 末尾相关条目。

## 2026-06-25：更新 CapacityReport 子模块指针（添加来源位置）

- CapacityReport CellData 图形化映射中，“添加来源”按钮移动到主键表达式输入区下方右侧，点击后自动滚动到新增来源。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：更新 CapacityReport 子模块指针（数据处理卡片并排）

- CapacityReport 数据处理页容量数据与 CellData 卡片在宽屏下左右并排展示，均有左上角数据类型和右上角说明按钮；容量数据说明补充启用 CellData 后会先刷新 CellData。
- 主仓库 gitlink 更新到 `metrix-integration` 的对应提交；推送时需同步推送 CapacityReport 子模块分支。

## 2026-06-25：修复 Windows 默认后端测试临时目录

- `pytest.ini` 默认参数改为 `-q -p no:cacheprovider --basetemp=runtime/pytest`，避免 Windows 环境中系统临时目录 `pytest-of-Administrator` 或旧的 `server/.pytest-temp` 权限/锁定问题导致默认 `python -m pytest` 全量失败。
- `runtime/` 已在根 `.gitignore` 中忽略，测试临时目录不会污染源码提交；禁用 pytest cacheprovider 后也不会再写入 `.pytest_cache`。
- 验证：`.venv\Scripts\python.exe -m pytest` 通过 65 passed；Metrix Web `npm run typecheck` 通过；CapacityReport 前端 `npx vue-tsc --noEmit --noUnusedLocals --noUnusedParameters` 通过；Metrix 后端与 CapacityReport 后端 `compileall` 均通过。

## 2026-06-25：更新 CapacityReport 子模块指针（后端日志清理）

- CapacityReport 子模块提交 `3147484` 将历史清理、处理日志写入失败和自动历史保留失败的直接 `print()` 诊断改为模块级 `logging`，减少后台 stdout 噪音，同时保留必要警告。
- `app/main.py` 的启动横幅输出保留，用于命令行直接启动时显示版本、配置更新时间和访问地址。
- 验证：CapacityReport 后端 `compileall` 通过；CapacityReport 前端 `npx vue-tsc --noEmit --noUnusedLocals --noUnusedParameters` 通过。
 
## 2026-06-25：修复 Metrix Web 构建根路径

- `web/vite.config.ts` 显式设置 `root: ROOT_DIR`，避免 Codex/Windows 沙箱中 Node 当前目录与真实 `E:` 盘配置路径不一致时，Vite 7.3.5 在打包 HTML 阶段把 `index.html` 解析成绝对输出名并导致 `npm run build` 失败。
- 验证：Metrix Web `npm run build` 通过；`.venv\Scripts\python.exe -m pytest` 通过 65 passed；Metrix 后端 `compileall` 通过；CapacityReport 后端 `compileall` 与前端 `npm run build` 均通过。
- 本轮同步清理 CapacityReport 上传保存路径：容量数据上传与 CellData 本地上传统一使用安全相对路径归一化，过滤空路径、`.` 与 `..` 片段，保留合法的文件夹层级。

## 2026-06-25：脚本压缩包安全解压与连接测试线程池化

- Metrix 脚本模块上传 ZIP/RAR/7Z 时不再直接信任压缩包路径：ZIP 改为逐成员安全写入，拒绝绝对路径、Windows 盘符、`..` 与符号链接；7Z/RAR 解压前先校验成员路径，staging 复制阶段也拒绝符号链接，避免恶意归档条目越界写入或被复制进脚本工作区。
- 新增脚本上传回归用例，验证包含 `../escape.py` 的 ZIP 返回 `error.scriptArchiveInvalid`，且项目工作区保持为空。
- CapacityReport 子模块将 `/api/remote/test`、`/api/config/cell-data/remote/test`、`/api/config/cell-data/mysql/test` 从 `async def` 改为同步接口，让 FastAPI 在线程池中执行 FTP/SFTP、Metrix HTTP 与 MySQL 连接测试，避免慢连接阻塞事件循环。
- 验证：主仓 `.venv\Scripts\python.exe -m pytest -q` 通过 66 passed；Metrix Web `npm run build` 通过；主仓后端 `compileall` 通过；CapacityReport 后端 `compileall` 与前端 `npm run build` 通过。`web/dist` 与 `CapacityReport/frontend/dist` 已清理；`.pytest_cache` 和 `server/.pytest-temp` 因当前权限/策略拒绝递归删除而保留。
