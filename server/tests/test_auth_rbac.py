...
def test_app_modules_register_permissions_routers_and_openapi_filters():
    ...
    assert [module.key for module in modules] == ["core", "storage", "demo_crud"]
    ...
    assert {spec.code for spec in get_page_permission_specs()} >= {"route:dashboard", "route:users", "route:storage", "route:demo_crud"}
    assert {spec.resource for spec in get_resource_permission_specs()} >= {"user", "role", "announcement", "storage", "demo_item"}
...
def test_app_modules_can_be_filtered_by_environment(monkeypatch):
    ...
    monkeypatch.setenv("METRIX_DISABLED_MODULES", "demo_crud,storage")
