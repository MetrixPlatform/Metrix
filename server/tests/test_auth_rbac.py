...
def test_app_modules_register_permissions_routers_and_openapi_filters():
    ...
    assert [module.key for module in modules] == ["core", "storage", "database", "containers", "scripts", "demo_crud"]
    ...
    assert "app.modules.scripts.models" in models_by_module["scripts"]
    ...
    assert "scripts" in get_openapi_hidden_tags()
    assert "/api/users" in get_openapi_hidden_path_prefixes()
    assert "/api/scripts" in get_openapi_hidden_path_prefixes()
    ...
    assert any(getattr(router, "prefix", None) == "/api/scripts" for router in load_module_routers())
